...
namespace chaos::graphics {
    ...
    bool Renderer::initialize(windows::Window* in_mainWindow)
    {
        HWND hwnd = glfwGetWin32Window(in_mainWindow->_glfwWindow);
        ...
        // create BitmapRenderTarget from HwndRenderTarget
        this->_hwndRenderTarget->CreateCompatibleRenderTarget(D2D1::SizeF(1000, 1000), &this->_bitmapRenderTarget);

        // create solid color brushs, whose properties update by any frame
    ...
    inline bool Renderer::initialize(windows::Window& in_mainWindow)
    {
        return this->initialize(&in_mainWindow);
    }
    ...
    bool Renderer::registerSubWindow(windows::Window* in_subWindow)
    {
        // EMERGENCY TODO: create a new viewport(BitmapRenderTarget) for the sub-window
    ...
    bool Renderer::registerSubWindow(windows::Window& in_subWindow)
    {
        return this->registerSubWindow(&in_subWindow);
    }
    ...
    void Renderer::SetBrushColor(Color in_brushColor)
    {
    ...
    void Renderer::SetFillBrushColor(Color in_fillBrushColor)
    {
        this->_fillBrush->SetColor(D2D1::ColorF(in_fillBrushColor.r, in_fillBrushColor.g, in_fillBrushColor.b));
    }
    ...
    void Renderer::_pushTransform(vec2<float> pivot, float rotation, vec2<float> scale)
    {
        ...
        if (this->_transformMatrixHeap.size() > 0)
            _matrix = this->_transformMatrixHeap.back() * _matrix;    // old * new
