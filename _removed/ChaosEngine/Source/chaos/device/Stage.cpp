...
namespace chaos::device {
    ...
    void Stage::registerWindow(windows::Window* in_window)
    {
        in_window->stage = this;
        this->windows.push_back(in_window);
    }
    ...
    void Stage::unregisterWindow(windows::Window* in_window)
    {
        for (size_t i = 0; i < this->windows.size(); i++) {
            if (this->windows.at(i) == in_window)
    ...
    void Stage::unregisterWindow(windows::Window& in_window)
    {
        this->unregisterWindow(&in_window);
    }



    void Stage::registerScene(Scene* in_scene)
    {
        if (!in_scene) return;
        for (auto& _scene : this->_scenes) {
            if (_scene) if (_scene->nameId == in_scene->nameId) return;  // scene already registered
        }
        in_scene->stage = this;
        this->_scenes.push_back(in_scene);
    }



    inline void Stage::registerScene(Scene& in_scene)
    {
        this->registerScene(&in_scene);
    }
    ...
    inline bool Stage::switchScene(std::string in_sceneName)
    {
        for (auto& _scene : this->_scenes) {
            if (_scene->nameId == in_sceneName) {
                this->_preparedScene = _scene;

                log::OutputStream _out("WARN");
                _out.push("Stage -> entered scene -> \"")->push(in_sceneName)->push("\"");
                this->_logger.print(_out);
    ...
    inline void Stage::switchScene(Scene* in_scene)
    {
        if (!in_scene) return;
        this->switchScene(in_scene->nameId);
    }
