...
namespace chaos::object {
    ...
    void Actor::onMessage(device::ActorMessage& message)
    {
    ...
    bool Actor::attachToActor(Actor* in_parentActor)
    {
        if (!in_parentActor) return false;
        if (!in_parentActor->scene) return false;

        if (in_parentActor->scene != this->scene) return false;  // the new parent and this actor are NOT within the same scene

        if (this->_parentActor) this->detachFromActor();
        in_parentActor->_childActors.push_back(this);
        this->_parentActor = in_parentActor;
        return true;
    }
